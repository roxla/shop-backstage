  ...
  methods:{
    submitForm(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          let port = 'login/merchantLogin'
          let obj = {
            merchantMobile: this.ruleForm.user,
            merchantPwd: this.ruleForm.pw
          }
          let transferData = this.axios.transferData(port,obj);
          transferData.then(res => {
            if(res.data.status == 200){
              sessionStorage.setItem("ShopId",res.data.data);
              this.$router.push({path: '/shopmain/shopindex'});
            }else{
